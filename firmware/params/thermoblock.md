...
Each side carries one term more than that, and they are different terms: the coffee side loses what the water drawn through it carries away, and the steam side loses the latent heat of the steam drawn off it. Both are advanced over a step by the exact solution for a constant duty, rather than by assuming the rate held. Per side:
...
| `water.latent_heat_j_per_ml` | What a millilitre of water costs to turn into vapour, which the steam side gives up for whatever is drawn off it | J/mL |
...
**Steam pressure.** Algebraic, from the steam block's temperature, zero until that mass reaches saturation, and this relation exactly whenever nothing is being drawn:
...
| `steam.pressure_fall_bar_per_ml` | How far a draw carries the path below that relation, per millilitre drawn, for as long as the draw is open | bar/mL |

While a draw is open the pressure is carried below that relation and no lower than nothing at all, and it is the relation again the moment the draw stops.
...
- **Steam quality.** What a steam draw costs the steam block is no longer among these omissions — the block gives up the latent heat of whatever is drawn off it, and the pressure comes away from that block's temperature while the wand is open. Nothing about the steam itself follows from either. This description reports a pressure and a temperature and has no notion of a delivered quantity or of its steam quality, so nothing here says how much steam is available or how wet what leaves the wand is.
