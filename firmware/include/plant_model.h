...
/*
 * Advance the instance by `interval_millis` under the given actuation.
 *
 * Returns false and changes nothing when an argument is null, when the
 * interval is zero, or when an actuation channel exceeds
 * PLANT_ACTUATION_FULL_SCALE -- a caller must treat a refusal as "the model
 * did not move", not as "the model advanced with the actuation clamped".
 *
