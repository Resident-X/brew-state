...
| `correction_read_only_fixture` | `PLANT_SUPPORT_UNVERIFIED` | — |
...
| `src/plant/correction_read_only_fixture/` | A structure that models nothing, alongside `fixture`, `flow_fixture` and `outlet_only_fixture`. Answers a *read* of both names the estimator reaches its one reconstructed value by -- the state the reconstruction is held as and the state its correction is applied through -- and refuses the *write* the correction ends with. That is the one pairing shape a structure could still be admitted on after `SOL-ADMISSION-PROVES-FULL-PAIRING` widened the admission check, because that check only ever read; `SOL-ADMISSION-PROVES-CORRECTION-WRITABLE` is what refuses it instead. |
...
| `test/test_estimator_correction_read_only_fixture/` | `estimator_init` asked directly for the admission a structure answering a read of the correction-target state and refusing its write is refused by -- the one case no other suite's structure can drive. |
