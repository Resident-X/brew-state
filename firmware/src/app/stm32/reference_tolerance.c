/*
 * Where the carried tolerance declaration's bytes and their length come from.
 *
...
/*
 * Generated into the build directory from the tolerance declaration the
 * environment declares, and put on the include path by the script that
 * generates it. It defines reference_tolerance; there is deliberately no copy
 * of it in the source tree.
 */
#include "reference_tolerance_bytes.h"

const size_t reference_tolerance_length = sizeof(reference_tolerance);
