 ...
 * with what it had rather than with a threshold read and the rest of a
 * future line left at whatever the memory contained.
 */
...
/*
 * The range the ready-pressure threshold may be declared inside, in
 * thousandths of a bar.
 *
 ...
 * where the design should set the threshold; both are chosen to admit any
 * figure a reader could mean by "the steam side is ready" and refuse a digit
 * typed in the wrong place.
 */
#define STEAM_CONTROL_DECLARATION_READY_PRESSURE_LOW_MILLI_BAR 100
#define STEAM_CONTROL_DECLARATION_READY_PRESSURE_HIGH_MILLI_BAR 15000
                                    ...
                                    steam_control_declaration_error_t *error)
{
    ...
    bool ready_pressure_given = false;
    ...
    while (cursor < limit) {
        ...
        if (!spans_word(name_begin, name_end, STEAM_CONTROL_DECLARATION_READY_PRESSURE_WORD)) {
            report(error, STEAM_CONTROL_DECLARATION_UNKNOWN, line_number, name_begin, name_length);
            return false;
        }
        if (ready_pressure_given) {
        ...
        if (!spans_word(unit_begin, figure_end,
                        UNIT_WORDS[STEAM_CONTROL_DECLARATION_UNIT_MILLI_BAR])) {
            report(error, STEAM_CONTROL_DECLARATION_UNIT_MISMATCH, line_number, name_begin,
                   name_length);
            return false;
        }

        if (figure < (long)STEAM_CONTROL_DECLARATION_READY_PRESSURE_LOW_MILLI_BAR ||
           figure > (long)STEAM_CONTROL_DECLARATION_READY_PRESSURE_HIGH_MILLI_BAR) {
            report(error, STEAM_CONTROL_DECLARATION_OUT_OF_RANGE, line_number, name_begin,
                   name_length);
            return false;
        }

        staging.ready_pressure_milli_bar = (int32_t)figure;
        ready_pressure_given = true;
    }

    if (!ready_pressure_given) {
        report(error, STEAM_CONTROL_DECLARATION_MISSING, 0u,
               STEAM_CONTROL_DECLARATION_READY_PRESSURE_WORD,
               strlen(STEAM_CONTROL_DECLARATION_READY_PRESSURE_WORD));
        return false;
    }
