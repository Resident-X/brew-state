...
- Water level sensor in the tank (magnet/reed type). This is read as `S6`, the schematic's `WAT` line — **the schematic's `S6`, which is not the panel
board's `S6`**: the panel silkscreens its own `S1`-`S10` for the five keys wired as parallel pairs, so
the same designator names a tank-level switch on one sheet and the second switch of the `PWR` pair on
the other. [`connector-pinouts.md`](connector-pinouts.md) sets the collision out in full. Reading on
with the schematic's numbering: on sheet 3-3 `S6`, `S7` and `S8` are drawn as plain switch symbols against `S1`-`S5`'s pushbutton symbols, and all three are held up through 1 kΩ resistors alongside the panel buttons. *(Provenance: owner's direct account that the two knob microswitches and a third switch run back on their own wires rather than through the front-panel harness, corroborated by the symbol distinction on the scan. Not yet bench-verified.)*
...
- Two stepper-driven analogue gauges (espresso "resistance to pour", milk temperature), both driven by a `VID66-08` two-channel stepper motor driver IC which sits on the **gauge assembly itself**, not on the main board: the machine has been opened and both that part and its decoupling capacitor were read off the gauge board directly, which also fits the nine-conductor harness closing only if the driver is local. The board silkscreens this part `U8`; the circuit diagram calls it `U9` and gives `U8` to the `HT66F50` instead — the two are swapped, and [`connector-pinouts.md`](connector-pinouts.md) reconciles them. At power-on the machine sweeps both needles to full scale and back to zero, so both movements have hard end stops and are homed against them. The driver is wired to the two motors (`MOT1` and `MOT2`) and commanded over direct control lines from the main board. Harness allocation, and what those control lines ask for, are in [`connector-pinouts.md`](connector-pinouts.md). *(Provenance: the driver's identity and location traced from the opened machine; the circuit's topology read from service manual circuit diagram sheet 3-3.)* This settles a previously open alternative: the gauges are reached through a dedicated driver IC, not by a step-and-direction pulse train into bare motor windings, and not over a serial link to a separate driver board.
...
**The control silicon is not on the front panel, and the two front assemblies are not alike.** Both Holtek parts — `U3`, which drives the heaters, pumps and valves and reads zero-cross, and `U8`, which reads the keys and thermistors and drives the indicators, the gauges and the buzzer — sit on the main control board. The **button and indicator panel** is passive: ten switches in five parallel pairs, eight indicator elements, the buzzer, and no active parts, reached by a nine-way harness. The **gauge assembly** is not passive: it carries the `VID66-08` stepper driver and the low-water indicator, and is reached by a second nine-way harness. *(Provenance: the button board traced from the physical spare part; the gauge assembly's contents now traced too — the machine was opened and the driver and its decoupling capacitor read off that board, retiring the earlier inference from their absence in main-board spare photographs. **Where the two microcontrollers sit is not carried by either of those**: that the panel holds no 28-pin part follows from the traced panel board, but that both parts sit on the main board rests on the owner's account and is not yet bench-verified. How the gauge harness's nine conductors are allocated remains inferred. Allocation and reasoning in [`connector-pinouts.md`](connector-pinouts.md).)*
