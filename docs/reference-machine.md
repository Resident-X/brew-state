...
- A low-water indicator on the gauge assembly, driven from an output pin sinking its cathode. *(Provenance: owner's direct observation of the running machine; harness reasoning in [`connector-pinouts.md`](connector-pinouts.md).)* It is switched rather than modulated — with a low tank it holds steady while the `PWR` element ramps, which is what places it on a supply separate from the button panel's gated rail. The circuit diagram draws this as a sixth `WATER` function of the button panel; it is not on that board. It is the visual half of a low-water annunciation whose audible half is the buzzer, and a replacement could keep one and drop the other.
